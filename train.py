...
class MyDeepNeuralNetwork:
    ...
    def softmax(self, data):
        log_softmax = data - np.max(data)  # For Numerical stability
        log_softmax -= np.log(np.sum(np.exp(log_softmax), axis=0))
        return np.exp(log_softmax)


    def apply_activation(self, activation_name, m):
        ...
        if activation_name in activation_functions:
            return activation_functions[activation_name](m)


    # Method to compute activation derivative
    def compute_activation_derivative(self, activation_name, p):
        if activation_name == "relu":
            return self.relu_derivative(p)
        elif activation_name == "sigmoid":
            return self.sigmoid_derivative(p)
        elif activation_name == "tanh":
            return self.tanh_derivative(p)
...
def parse_args():
    p = argparse.ArgumentParser(description="Train a neural network with specified hyperparameters.")
    p.add_argument("-wp", "--wandb_project", type=str, default="CS24M027_Deep_Learning_Assignment_1", help="Project name for Weights & Biases")
    p.add_argument("-we", "--wandb_entity", type=str, default="", help="Wandb Entity for tracking experiments")
    p.add_argument("-d", "--dataset", type=str, default="fashion_mnist", choices=["mnist", "fashion_mnist"], help="Dataset to use")
    p.add_argument("-e", "--epochs", type=int, default=2, help="Number of epochs to train")
    p.add_argument("-b", "--batch_size", type=int, default=64, help="Batch size for training")
    p.add_argument("-l", "--loss", type=str, default="cross_entropy", choices=["mean_squared_error", "cross_entropy"], help="Loss function")
    p.add_argument("-o", "--optimizer", type=str, default="nadam", choices=["sgd", "momentum", "nag", "rmsprop", "adam", "nadam"], help="Optimizer")
    p.add_argument("-lr", "--learning_rate", type=float, default=0.001, help="Learning rate")
    p.add_argument("-m", "--momentum", type=float, default=0.8, help="Momentum for SGD with momentum or NAG")
    p.add_argument("-beta", "--beta", type=float, default=0.99, help="Beta for RMSProp")
    p.add_argument("-beta1", "--beta1", type=float, default=0.8, help="Beta1 for Adam and Nadam")
    p.add_argument("-beta2", "--beta2", type=float, default=0.99, help="Beta2 for Adam and Nadam")
    p.add_argument("-eps", "--epsilon", type=float, default=1e-8, help="Epsilon for optimizers")
    p.add_argument("-w_d", "--weight_decay", type=float, default=0.0, help="Weight decay")
    p.add_argument("-w_i", "--weight_init", type=str, default="xavier", choices=["random", "xavier"], help="Weight initialization method")
    p.add_argument("-nhl", "--num_layers", type=int, default=4, help="Number of hidden layers")
    p.add_argument("-sz", "--hidden_size", type=int, default=128, help="Number of neurons in hidden layers")
    p.add_argument("-a", "--activation", type=str, default="relu", choices=["identity", "sigmoid", "tanh", "relu"], help="Activation function")
    return p.parse_args()
...
def main():
    ...
    def train_sweep():
        ...
        neural_network = MyDeepNeuralNetwork(784, 10, config=hyperparameter_defaults)
